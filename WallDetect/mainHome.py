...
class MainHomeScreen(MDScreen):
    ...
    def determine_summary_status(self):
        if (self.object_counts["crack"] == 0 and
                self.object_counts["dirty"] == 0 and
                self.object_counts["air bubbles"] == 0 and
                self.object_counts["not uniform"] == 0 and
                self.object_counts["peeling paint"] == 0):
            self.summary_status = "Cấp độ 1 (Tốt)"
        elif (self.object_counts["crack"] <= 2 and
              self.object_counts["dirty"] <= 2 and
              self.object_counts["air bubbles"] == 0 and
              self.object_counts["not uniform"] == 0):
            self.summary_status = "Cấp độ 2 (Khá)"
        elif (self.object_counts["crack"] <= 5 or
              self.object_counts["dirty"] <= 5 or
              self.object_counts["air bubbles"] > 0):
            self.summary_status = "Cấp độ 3 (Trung bình)"
        elif (self.object_counts["crack"] > 5 or
              self.object_counts["dirty"] > 5 or
              self.object_counts["air bubbles"] > 3 or
              self.object_counts["peeling paint"] > 3):
